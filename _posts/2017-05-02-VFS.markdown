...
这两个方法获取vsfmount对象，将新挂载的文件系统插入队列，接着安装新的文件系统到用户空间的目录树，当用户查找文件时，只要他发现这是一个挂载点，那么他就会到 mount_hashtable 这个哈希队列找到挂载到这个目录上的文件系统的 vfsmount 对象，继而找到文件系统的根目录，这样也就能继续寻找下去了，vfsmount是整个过程的关键线索。
